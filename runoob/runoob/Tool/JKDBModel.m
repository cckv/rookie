...
- (BOOL)saveOrUpdateByColumnName:(NSString*)columnName AndColumnValue:(NSString*)columnValue
{
    ...
    if (record) {
        ...
        if ([primaryValue intValue] <= 0) {
            ...
            self.errorId = [primaryValue integerValue];
            return [self update];
        }
    }else{
        return [self save];
...
- (BOOL)save
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        self.errorId = res?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
        NSLog(res?@"插入成功":@"插入失败");
    }];
    return res;
}
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            model.errorId = flag?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
            NSLog(res?@"插入成功":@"插入失败");
            if (!flag) {
                res = NO;
                *rollback = YES;
